...
def generate_platform_headers(secure_lcs, output_path, project_name,
                              bootstrap_size, bootstrap_dst_addr,
                              flash_addr_offset, smif_config):
    ...
    sys.exit(bin2hex(Path(output_path) / 'app_header.bin',
                     Path(output_path) / 'app_header.hex', int(flash_addr_offset, 16)))

def main():
    parser = argparse.ArgumentParser(allow_abbrev=False)
    parser.add_argument('-m', '--secure_lcs', required=False, type=bool, default=False,
    ...
    parser.add_argument('--bootstrap-dst-addr', required=False,
    ...
    generate_platform_headers(args.secure_lcs,
                              args.project_path, args.project_name,
                              args.bootstrap_size, args.bootstrap_dst_addr,
                              args.flash_addr_offset, args.smif_config)
